...
parser.add_argument('--task', type=str, default='Uniform-deblurring', help='image deblurring or super-resolution')###Gaussian-deblurring; Uniform-deblurring; Super-resolution
...
parser.add_argument('--noise_level', type=float, default=2**0.5, help='noise level of image')
    ...
    for name in test_imgs:
        ...
        with torch.no_grad():
            if algorithm == 'SAM_PROv1':###S(x) = x-s\nabla f(x)
                ...
                if name == 'butterfly':
                    opt={'alpha':4, 'beta':0.01, 'sigma':noise_level, 'sigma_f':sigma_f, 'K':2500, 'mu_0':1800, 'sf': Sf}
                else:

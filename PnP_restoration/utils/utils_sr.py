...
def p2o2(psf, shape):##########pytorch1.10版本
    '''
...
def p2o(psf, shape):##########pytorch1.7版本
    '''
    Convert point-spread function to optical transfer function.
    otf = p2o(psf) computes the Fast Fourier Transform (FFT) of the
    point-spread function (PSF) array and creates the optical transfer
...
def grad_solution(x, FB, FBC, FBFy, sf):###############torch 1.7
...
def grad_solution2(x, FB, FBC, FBFy, sf):###############torch 1.10
...
def pre_calculate_prox2(x, k, sf):
    '''
    Args:
        x: NxCxHxW, LR input
        k: NxCxhxw
